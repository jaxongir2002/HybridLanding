...
onMounted(() => {
  ...
  function updateColors(dt) {
    ...
    if (colorUpdateTimer >= 1) {
      colorUpdateTimer = wrap(colorUpdateTimer, 0, 0.3);
  ...
  function render(target) {
    ...
    if (target == null && config.TRANSPARENT) drawDisplay(fbo, width, height);
  }
